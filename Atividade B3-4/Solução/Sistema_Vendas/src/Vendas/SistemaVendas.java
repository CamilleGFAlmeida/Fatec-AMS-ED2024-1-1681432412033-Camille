...
public class SistemaVendas {
    ...
    public Map<Produto, Integer> totalVendasPorProduto() {
        ...
        for (Venda venda : listaVendas) {  // Laço 1 (For)
    ...
    // Recursividade - Comparar vendas entre períodos (dividir e conquistar)
    public double compararVendas(int inicio, int fim) {
        if (inicio == fim) {
            return listaVendas.get(inicio).getValorTotal();
        }
        int meio = (inicio + fim) / 2;
        double totalEsquerda = compararVendas(inicio, meio);  // Recursividade
        double totalDireita = compararVendas(meio + 1, fim);  // Recursividade
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nComparação de Vendas (Recursiva): R$ " + totalComparado);
    }
}
